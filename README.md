...
| `npm run test:e2e` | Runs the daily matrix: 38 Chromium functional cases, 28 browser-critical cases in Firefox/WebKit, and 10 visual cases. |
| `npm run test:e2e:functional` | Runs the 38 structural and interaction cases in Chromium without visual snapshots. |
| `npm run test:e2e:browser-critical` | Runs the 14 critical scenarios in Firefox and WebKit (28 executions). |
...
| `npm run test:e2e:cross-browser` | Runs all 38 functional scenarios in Chromium, Firefox, and WebKit (114 executions). |
| `npm run test:e2e:full` | Runs the complete 124-execution matrix, including visual coverage. |
...
Playwright starts an isolated production server on `127.0.0.1:3218`. Chromium runs all 39 functional scenarios and the approved visual coverage. Firefox and WebKit run the 15 scenarios tagged `@cross-browser` in the daily matrix; all 39 functional scenarios remain available in both engines through `test:e2e:cross-browser` and `test:e2e:full`. Visual snapshots remain restricted to Chromium on Windows.
...
GitHub Actions uses separate validation lanes. Pull requests into `integration/**` or `dev` run lint/build and the 39 Chromium functional cases in parallel, then report the single required `CI dev gate`. Pull requests into `main` run lint/build, three independent one-worker functional jobs totaling 117 cross-browser executions, and the 10 Windows visual cases in parallel, then report the single required `CI release gate`. Vercel remains a separate required provider check. Manual runs use distinct check names and cannot replace either protected gate. Always verify results against the Pull Request's current head commit. Reports and failure artifacts are retained for seven days.

The expected targets under normal runner capacity are under 10 minutes for the `dev` gate and under 6 minutes for the release gate. The local gate commands use one Playwright worker to mirror each browser job and avoid resource-contention failures. The scheduled `CI metrics` workflow separates current-attempt time from end-to-end recovery time and reports job-start delay, critical-job execution, reruns, cancellations, and failures. See `docs/delivery-runbook.md` for check-state diagnosis, incident handling, and reliability thresholds.
